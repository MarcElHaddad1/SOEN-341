  ...
  const demo = [
    {
      ...
      category: 'Career',
      org: 'SCS',
      desc: 'Meet recruiters from tech companies.',
      cap: 80,
      attendees: [],
      // waitlist starts empty
      waitlist: [],
      createdBy: { email: s.email, name: s.name, role: s.role }
    },
    {
      id: crypto.randomUUID(),
      title: 'Hack Night',
      ...
      category: 'Coding',
      org: 'CS Club',
      desc: 'Bring your laptop; pizza provided.',
      cap: 40,
      attendees: [],
      waitlist: [],
      createdBy: { email: s.email, name: s.name, role: s.role }
    },
    {
      ...
      category: 'Health',
      org: 'Wellness',
      desc: 'Relax and stretch with peers.',
      cap: 30,
      attendees: [],
      waitlist: [],
      createdBy: { email: s.email, name: s.name, role: s.role }
...
function waitlistCount(e) {
  return getWaitlist(e).length;
}
function isOnWaitlist(e, email) {
  return getWaitlist(e).some(w => w.email === email);
}
...
function showDetails(id) {
  ...
  const waitlistBtn = q('#waitlist-btn');
  const waitInfo = q('#waitlist-info');
  const me = currentUser();
  const alreadyClaimed = hasTicket(e, me.email);
  const full = seatsLeft(e) <= 0;
  const onWait = isOnWaitlist(e, me.email);
  const count = waitlistCount(e);

  const canClaim = isStudent() && seatsLeft(e) > 0 && !alreadyClaimed;
  claimBtn.disabled = !canClaim;
  claimBtn.dataset.id = e.id;

  // Waitlist button only shows when max capacity is reached
  if (waitlistBtn) {
    if (isStudent() && full && !alreadyClaimed) {
      waitlistBtn.style.display = 'inline-block';
      waitlistBtn.disabled = onWait;
      waitlistBtn.dataset.id = e.id;
    } else {
  ...
  // Displays how many students are in the waitlist (Student UI)
  if (waitInfo) {
    if (isStudent()) {
      if (alreadyClaimed) {
        waitInfo.textContent = 'You already have a ticket for this event.';
      } else if (full) {
        if (onWait) {
          waitInfo.textContent = `You are on the waitlist (${count} student(s) total).`;
        } else if (count > 0) {
          waitInfo.textContent = `${count} student(s) are currently on the waitlist.`;
        } else {
          waitInfo.textContent = 'Event is full. No one is on the waitlist yet.';
        }
      } else {
  ...
  // Waitlist pop up screen for admin and teachers
  let adminPopup = q('#attend-Popup');
  if (!adminPopup) {
    adminPopup = document.createElement('div');
    adminPopup.id = 'attend-Popup';
    adminPopup.style.marginTop = '12px';
    q('.modal-card').appendChild(adminPopup);
  }
  ...
  if (isAdmin() || isTeacher()) {
    ...
    const waitRows = wl.map(w => `
      <li class="cal-item">
        <span>${w.name} &lt;${w.email}&gt;</span>
      </li>`).join('');

    const downloadBtnHtml = `<button id="download-btn" class="btn secondary" 
    ...
    adminPopup.innerHTML = `
      ...
      <h3 style="margin-top:12px;">Waitlist (${wl.length})</h3>
      <ul class="cal-list">${waitRows || '<li class="muted">No one on the waitlist.</li>'}</ul>`;
  } else {
    adminPopup.innerHTML = '';
  }
...
function wireGlobal() {
  ...
  // Join waitlist only when event is max capacity
  q('#waitlist-btn')?.addEventListener('click', () => {
    const id = q('#waitlist-btn').dataset.id;
    const me = currentUser();
    const events = loadEvents();
    const e = events.find(x => x.id === id);
    if (!e) return;
    if (!isStudent()) return toast('Only students can join the waitlist.');
    if (hasTicket(e, me.email)) return toast('You already have a ticket.');
    if (seatsLeft(e) > 0) return toast('Seats are available, claim a ticket instead.');
    if (isOnWaitlist(e, me.email)) return toast('You are already on the waitlist.');
  ...
  // Admin can remove attendee and promote user from waitlist
  modal?.addEventListener('click', (ev) => {
    if (ev.target.matches('[data-act="kick"]')) {
      ...
      if (wl.length > 0) {
        ...
        toast(`Attendee removed. ${next.name} moved from the waitlist to attendees.`);
      } else {
